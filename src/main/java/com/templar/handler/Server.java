...
import com.templar.param.ErrorMsg;
import com.templar.param.MsgType;
import com.templar.thread.ServerSocketThread;
...
public class Server {
    ...
    public void start() {
        try {
            ...
            alive = true;
            logger.info("服务端接口启动，开始监听: " + port + "端口");
            while (alive) {
                Socket socket = server.accept();
                Thread socketThread = new ServerSocketThread(socket, this);
                socketThread.start();
                count.incrementAndGet();
                logger.info("欢迎 user-" + socketThread.getId() + " 加入聊天室 " +
                        "当前用户共有 " + count + "人");
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void socketDisconnect(ServerSocketThread socket) {
        ...
        logger.info("User-" + socket.getId() + " 退出聊天室，当前用户共: " + current);
    ...
    public synchronized void addSocketToRoom(ServerSocketThread socket, String roomName) throws Exception {
        ...
        logger.info("user-" + socket.getId() + " 加入聊天室： " + roomName);
    }
    ...
    public synchronized void createChatRoom(ServerSocketThread socket, String roomName) throws Exception {
        ...
        sockets.add(socket);
        rooms.put(roomName, sockets);
        logger.info("user-" + socket.getId() + " 创建聊天室: " + roomName);
    }
    ...
    public synchronized void removeSocketFromRoom(ServerSocketThread socket, String roomName) throws Exception{
        ...
        logger.info("user-" + socket.getId() + " 退出聊天室： " + roomName);
        if (sockets.size() == 0) {
